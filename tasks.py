...
# Some default values
PACKAGE_NAME = "ta_lib"
ENV_PREFIX = "case-study"
ENV_PREFIX_PYSPARK = "case-study"
NUM_RETRIES = 10
SLEEP_TIME = 1
...
def check_setup_prerequisites(c):
    ...
    if _failed:
        raise RuntimeError(
            f"Failed to find the following binaries in path : {_failed}"
        )
...
def setup_env_legacy(c, platform=PLATFORM, env=DEV_ENV, force=False):
    ...
    if not op.isfile(env_file):
        raise ValueError(
            f"""The conda env file is not found : "{env_file}" """
        )
...
def _setup_env_common(
    ...
    with py_env(c, env_name):
        ...
        c.run(f"""python -m pip install -e "{HERE}" """)
        c.run(
            f"""echo "To activate this env use: conda activate {env_name}" """
        )
...
def _setup_env_common_usecase(
    c, env_name, platform=PLATFORM, env=DEV_ENV, force=False
):
    """
...
@task(
    help={
        "platform": (
            "Specifies the platform spec. Must be of the form " "``ct-core``"
        ),
        "env": "Specifies the environment type. Must be one of ``{dev|test|run}``",
        "force": "If ``True``, any pre-existing environment with the same name will be overwritten",
...
def setup_env(
    ...
    if usecase:
        if usecase == "tpo":
            usecase_file = op.abspath(
                op.join(CONDA_ENV_FOLDER, f"ct-tpo-{env}.yml")
            )
        elif usecase == "mmx":
            usecase_file = op.abspath(
                op.join(CONDA_ENV_FOLDER, f"ct-mmx-{env}.yml")
            )
        elif usecase == "ebo":
            usecase_file = op.abspath(
                op.join(CONDA_ENV_FOLDER, f"ct-ebo-{env}.yml")
            )
        elif usecase == "rtm":
            usecase_file = op.abspath(
                op.join(CONDA_ENV_FOLDER, f"ct-rtm-{env}.yml")
            )
        elif usecase == "reco":
            usecase_file = op.abspath(
                op.join(CONDA_ENV_FOLDER, f"ct-reco-{env}.yml")
            )
        else:
    ...
    if usecase:
        _setup_env_common_usecase(
            c, env_name, platform=platform, env=env, force=force
        )
        with py_env(c, env_name):
            # install the current package
            out_upd = c.run(
...
def _addon_file_paths(platform, env, addon_list):
    ...
    for addon in addon_list:
        ...
        addon_path_without_platform = op.join(
            PIP_REQ_FOLDER, f"{addon}-{env}.txt"
        )
...
def _build_docker_image(c):
    with tempfile.TemporaryDirectory() as tempdir:
        ...
        shutil.copytree(
            op.join(HERE, "production"), op.join(tempdir, "production")
        )
        shutil.copytree(
            op.join(HERE, "src", "ta_lib"), op.join(tempdir, "src", "ta_lib")
        )
        shutil.copyfile(
            op.join(HERE, "setup.py"), op.join(tempdir, "setup.py")
        )
        shutil.copyfile(
            op.join(HERE, "setup.cfg"), op.join(tempdir, "setup.cfg")
        )
        build_context = tempdir
        c.run(
            f"""docker build -f "{docker_file}" -t {tag} "{build_context}" """
        )
...
def setup_ci_env(c, platform=PLATFORM, force=False):
    ...
    force_flag = "" if not force else "--force"
    env_file = op.abspath(
        op.join(CONDA_ENV_FOLDER, f"ct-full-{platform}-{env}.yml")
    )
...
def _get_expected_env_list(env_files):
    ...
    expected_list.append(
        "ta-lib=={}".format(get_package_version(SOURCE_FOLDER))
    )
...
def validate_env(
    ...
    # actual validation
    package_diff_keys = list(
        set(expected_dict.keys()) - set(installed_dict.keys())
    )
    package_diff = [
        v for (k, v) in expected_dict.items() if k in package_diff_keys
    ]
...
@task(name="docs")
def build_docs(
    c, platform=PLATFORM, env=DEV_ENV, regen_api=True, update_credits=False
):
    env_name = _get_env_name(env)
    with py_env(c, env_name):
        if regen_api:
...
def start_docs_server(c, ip="127.0.0.1", port=8081):
    ...
    c.run(
        f"python -m http.server --bind 127.0.0.1 "
        f"--directory docs/build/html {port}"
    )
